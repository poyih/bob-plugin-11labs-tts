...
| 6 | **（2026-09-25 新增）** `eleven_v3_conversational` 的 `/v1/models` 元数据：字符上限、`can_use_style` / `can_use_speaker_boost`、`languages` | **2026-09-25 已验（payg；Key 需勾选 Models: Read，否则 `/v1/models` 回 401 `missing_permissions`）**。`/v1/models`：`max_characters_request_free_user` = `subscribed_user` = **5000**；`can_use_style=false`；**`can_use_speaker_boost=true`**（与 v3 的 false 不同）；`languages` **74 种，与 v3 逐一相同**；`can_do_text_to_speech=true`；`model_rates.character_cost_multiplier=0.5`（按字计费是 v3 的一半）；`concurrency_group=standard_eleven_v3`。实打：合成 200；+ zh / + af → 200；下发 speed / style / use_speaker_boost → 200。另记：Key 缺 `models_read` 时想免费夹逼上限走不通——非法 output_format（403 `invalid_output_format`）和不存在的 Voice ID（404 `voice_not_found`）都先于字符上限校验 | **已验**。`MODELS` 5000、`MODEL_LANGUAGES` null 维持；`MODEL_SETTINGS` 改为 `use_speaker_boost: true`（style 仍 false）；`info.json` 的 style / speakerBoost 文案、`test_plugin.js` 第 27 组同步 |
...
- v3_conversational = 与 v3 完全相同的 74 种（2026-09-25 `/v1/models` 实测逐一相同；+ zh / + af 实打 200），同样置 null。
