...
| 6 | **（2026-09-25 新增）** `eleven_v3_conversational` 的 `/v1/models` 元数据：字符上限、`can_use_style` / `can_use_speaker_boost`、`languages` | **2026-09-25 部分实测（payg，仅 TTS 权限的 Key）**：`/v1/text-to-speech` + 该模型 → **200**；+ `speed` / `style` / `use_speaker_boost` → 200（与 v3 一样静默接受）；+ `zh` → 200；+ `af`（flash_v2_5 必 400 的探针语言）→ 200。**`/v1/models` 元数据仍未取到**：所用 Key 缺 `models_read` 权限，GET `/v1/models` 回 401 `missing_permissions`（`/v1/voices` 同理缺 `voices_read`，脚本自动退到默认 Voice ID）。试过绕开 `models_read` 免费夹逼上限——每个请求带非法 output_format 让它必然失败、不计费，看先报哪个错：服务端先回 403 `invalid_output_format`，排在字符上限校验之前；换成不存在的 Voice ID 做守卫也一样，404 `voice_not_found` 先于字符上限校验。两条免费夹逼路都不通。`config.js` 三张表因此仍按 v3 同档：5000 字、不发 style / speaker_boost、语言集 null。假设错了的后果都可控：上限偏高 → API 回 400 `max_character_limit_exceeded`，插件已正确报错；门控偏严 → 漏发一个字段；语言集偏宽 → 该语言 400 `unsupported_language` | **待补元数据**：用带 `models_read` 权限的 Key 跑 `verify_api.py --only models`（只想拉元数据可单独 GET `/v1/models`，0 credits），按打印的 `max_chars` / `can_use_*` / 「比 v3 少」改 `config.js` 的 `MODELS` / `MODEL_SETTINGS` / `MODEL_LANGUAGES`，改完把本行标成已验 |
...
- v3_conversational = 按 v3 全支持处理（2026-09-25 实打 + zh → 200、+ af → 200，与 v3 一致；完整 `languages` 列表待带 `models_read` 的 Key，见 P1#6）。
