...
Bob（macOS 翻译软件）的 ElevenLabs 语音合成插件。当前 v1.0.5，功能可用。
...
```bash
make test                              # 49 项断言，用 macOS 自带 jsc（Bob 同款引擎），不联网
python3 scripts/verify_api.py          # 拿真实 Key 打 ElevenLabs，6 组探针，约 30~40 credits
python3 scripts/verify_api.py --dry-run
python3 scripts/resolve_voices.py      # 解析官方替换表的 19 个新音色 → 真 voice_id（只读，不耗额度）
